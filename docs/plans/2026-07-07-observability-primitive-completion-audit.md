...
| Portable setup doctor exists for MacBook/VPS/Docker readiness | Proven locally | `experiments/2026-07-08--langfuse--portable-setup-doctor`; `scripts/langfuse-observability-doctor.sh` | Read-only preflight reports official Claude/Codex prerequisites, `LANGFUSE_*` set/missing state, JSONL/Syntropic137 fanout, MCP presence, and the focused OTLP noise-guard test without printing credentials. |
