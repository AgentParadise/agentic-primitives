...
docs:
    ...
    cd docs-site-fuma && npm install --silent && npm run dev -- -p 4321

[group('docs')]
[windows]
docs:
    Write-Host "Starting docs site (Fumadocs) on http://localhost:4321..." -ForegroundColor Yellow
    Set-Location docs-site-fuma; npm install --silent; npm run dev -- -p 4321
...
docs-build:
    ...
    cd docs-site-fuma && npm install --silent && npm run build
...
docs-build:
    ...
    Set-Location docs-site-fuma; npm install --silent; npm run build
