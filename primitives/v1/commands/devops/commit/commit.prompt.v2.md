...
⚠️ **NEVER use `git add -A` or `git add .`** - These commands can accidentally stage sensitive files, secrets, or unrelated changes.

**Instead, stage files selectively:**
...
# Or stage interactively to review each change
git add -p
...
If nothing is staged, review the changes and stage only the files related to this commit.
