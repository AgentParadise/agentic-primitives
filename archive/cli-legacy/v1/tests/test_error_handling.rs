...
fn test_invalid_primitive_structure() {
    ...
    let invalid_path = repo_path.join("primitives/v1/prompts/agents/testing/invalid-agent");
    fs::create_dir_all(&invalid_path).unwrap();
    fs::write(invalid_path.join("invalid-agent.prompt.v1.md"), "Content").unwrap();
    // No metadata file
...
fn test_missing_required_files() {
    ...
    let incomplete_path = repo_path.join("primitives/v1/prompts/agents/testing/incomplete-agent");
...
"#;
    ...
    .failure();
    // Should fail - missing version files and invalid schema
}
...
fn test_malformed_yaml() {
    ...
    let malformed_path = repo_path.join("primitives/v1/prompts/agents/testing/malformed-agent");
...
"#;
    ...
    fs::write(
        malformed_path.join("malformed-agent.prompt.v1.md"),
        "Content",
    )
    .unwrap();
...
fn test_install_without_build() {
    ...
    run_cli_command(&["install", "--provider", "claude"], Some(repo_path))
        .failure()
        .stderr(
            predicate::str::contains("build directory").or(predicate::str::contains("not found")),
        );
}
