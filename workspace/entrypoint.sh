...
# Tool homes that outgrow the $HOME tmpfs live under /workspace instead.
#
...
#
# WHY SYMLINKS AND NOT ONLY ENV. syntropic137 already exports cache variables
# for exec'd commands (its #1133), but task runners that sanitise the
# environment drop them: turbo's strict env mode stripped RUSTUP_HOME in the
...
#
# An existing directory is left alone (a caller that mounted one meant it).
# ~/.cargo keeps its old contract - writable by the agent - because claude-cli
# points CARGO_HOME at a read-only /usr/local/cargo and relies on it.
TOOL_HOMES_ROOT=/workspace/.tools

# Fail closed on a planted path. /workspace is a bind mount, so anything that
# wrote into it before this ran (a reused directory, a hostile checkout) could
# have left `.tools` or one of its children as a symlink pointing somewhere
# else - including back onto this tmpfs, or into a directory that is later
# uploaded. mkdir -p would follow it silently. Refuse instead: a workspace that
# starts with a symlinked tool home is not one to run in.
_unsafe_tool_path() {
    [ -L "$1" ] || { [ -e "$1" ] && [ ! -d "$1" ]; }
}
if _unsafe_tool_path "$TOOL_HOMES_ROOT"; then
    echo "[entrypoint] refusing unsafe tool-home root (symlink or non-directory): $TOOL_HOMES_ROOT" >&2
    exit 1
fi
...
for link in \
    ...
    if _unsafe_tool_path "$target"; then
        echo "[entrypoint] refusing unsafe tool-home target (symlink or non-directory): $target" >&2
        exit 1
    fi
    mkdir -p "$target"
...
# Size cargo's parallelism to the container, not the host.
#
# WHY. Workspaces run with a CPU quota (2 CPUs on the syntropic137 selfhost)
# and a 4 GB memory limit, but nproc - and therefore cargo's default job count -
# reports every host core (16 there). A wgpu test build then ran 16 rustc and
# linker processes at once and the kernel OOM-killed ld (measured 2026-09-17:
# `collect2: fatal error: ld terminated with signal 9 [Killed]`).
#
# WHY A CONFIG FILE AND NOT CARGO_BUILD_JOBS. Task runners that sanitise the
# environment (turbo's strict mode) drop the variable before cargo sees it;
# cargo always reads $CARGO_HOME/config.toml. An existing file is left alone,
# and an explicit CARGO_BUILD_JOBS still wins, because cargo ranks env over
# config.
...
if [ ! -e "$_cargo_config" ] && [ ! -L "$_cargo_config" ]; then
    ...
    if [ -r /sys/fs/cgroup/cpu.max ]; then
        ...
        # cgroup v1: quota is -1 when unlimited, which the numeric check below rejects
    ...
    if [ -n "$_jobs" ] && [ "$_jobs" -ge 1 ] 2>/dev/null; then
        printf '# Written by the workspace entrypoint: jobs sized to the container CPU quota.\n[build]\njobs = %s\n' "$_jobs" > "$_cargo_config"
    fi
