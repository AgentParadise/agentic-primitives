...
| Actual CLI query path | `runs/local-receiver-query/summary.txt`, `runs/local-receiver-query/captured-request.redacted.json`, `runs/local-receiver-query/query-response.json` | Passed locally after running the local receiver: CLI issued GET to Observations API v2, Basic auth matched synthetic credentials, and response JSON was parsed. |
