...
| Successful mock export reports a LangFuse UI trace URL | Mock transport test asserts one `/project/project-123/traces/<32_hex>` link | correct | `runs/langfuse-tests.txt` |
...
- The next real-backend smoke must prove the reported trace id is visible or
  queryable in LangFuse, not merely that the local mock accepted transport.
