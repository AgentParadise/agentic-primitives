...
| MCP trace query | `runs/langfuse-mcp-trace-query/summary.txt`, `runs/langfuse-mcp-trace-query/response.json`, `runs/langfuse-mcp-trace-query/summary.json`, `plugins/observability/mcp/langfuse_server.py` | Passed against local LangFuse Docker Compose: MCP `tools/call` for `agentic_langfuse_trace_summary` returned `isError=false` and the compact trace summary for `run-88868068` with harness `codex`, provider `openai`, model `gpt-5.5`, 15943 total tokens, and calculated cost `0.080015`. |
